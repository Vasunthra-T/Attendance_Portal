...
public class ApprovalServiceImpl implements ApprovalService {
    ...
    @Override
    public List<ApprovalResponse> showApproval(String empCode, Date fromDate, Date toDate, Approval.Status status) {
        List<Timesheet> subordinates = timesheetRepository.findByEmpCodeAndApproval_StatusAndWorkingDateBetween(empCode,status, fromDate, toDate);

        List<ApprovalResponse> myList = new ArrayList<>();
        for (Timesheet time : subordinates) {
            ApprovalResponse correctTimesheet = new ApprovalResponse(time.getEmpCode(), time.getEmpName(), time.getWorkingDate(), time.getProductiveHours(), time.getApproval().getInType(), time.getApproval().getPeriod(), time.getApproval().getStatus());
