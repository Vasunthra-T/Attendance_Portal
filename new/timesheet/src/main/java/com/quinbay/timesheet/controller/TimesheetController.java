...
public class TimesheetController {
    ...
    public List<TimesheetResponse> generateSummary(@RequestParam String empCode, @RequestParam(value = "fromDate",required =false ,defaultValue = "#{T(java.time.LocalDate).now()}") @DateTimeFormat(pattern = "yyyy-MM-dd") Date fromDate,
                                                   @RequestParam(value = "toDate",required =false ,defaultValue = "#{T(java.time.LocalDate).now()}") @DateTimeFormat(pattern = "yyyy-MM-dd") Date toDate){
    ...
    public List<Simulator> getSimulatorDetails(@PathVariable String empCode, @PathVariable @DateTimeFormat(pattern = "yyyy-MM-dd") Date workingDate){
        return timesheetService.getSimulatorDetails(empCode,workingDate);
    }

    @PutMapping("approveEmployee")
    public ResponseEntity<String> approveEmployee(@RequestParam String empCode, @RequestParam @DateTimeFormat(pattern = "yyyy-MM-dd") Date workingDate,
    ...
    public List<TimesheetResponse> generateSummaryByPaging(@RequestParam String empCode, @RequestParam(value = "fromDate",required =false ,defaultValue = "2016-01-01") @DateTimeFormat(pattern = "yyyy-MM-dd") Date fromDate,
                                                   @RequestParam(value = "toDate",required =false ,defaultValue = "#{T(java.time.LocalDate).now()}") @DateTimeFormat(pattern = "yyyy-MM-dd") Date toDate,
